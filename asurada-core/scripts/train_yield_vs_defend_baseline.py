...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "training" / "reports" / "yield_vs_defend_baseline"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the first yield-vs-defend baseline model.")
...
def main() -> int:
    ...
    df = df[df["primary_action_label"] == "DEFEND_WINDOW"].copy()
    df = df[df["yield_vs_fight_proxy_label"].isin(["defend", "yield_and_counter"])].copy()

    if df.empty:
        raise SystemExit("No defend-window rows available for training.")

    df["target_yield_and_counter"] = (df["yield_vs_fight_proxy_label"] == "yield_and_counter").astype(int)
    ...
    target_column = "target_yield_and_counter"
    ...
    model_path = args.output_dir / "yield_vs_defend_model_baseline.txt"
    summary_path = args.output_dir / "yield_vs_defend_baseline_report.json"
    ...
    summary = {
        ...
        "label_source": "yield_vs_fight_proxy_label",
