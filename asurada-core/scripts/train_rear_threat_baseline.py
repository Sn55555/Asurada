...
def default_dataset_path() -> Path:
    for dataset_name in ("phase2_dataset_v2_extended", "phase2_dataset_v1"):
        candidate = PROJECT_ROOT / "training" / "exports" / dataset_name / "tactical_features_v1.csv"
        if candidate.exists():
            return candidate
    return PROJECT_ROOT / "training" / "exports" / "phase2_dataset_v1" / "tactical_features_v1.csv"


DEFAULT_DATASET = default_dataset_path()
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "training" / "reports" / "rear_threat_baseline"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the first rear-threat baseline model.")
...
def main() -> int:
    ...
    df = df[df["official_gap_behind_s"].notna()].copy()
    df = df[df["official_gap_confidence_behind"] == "high"].copy()

    if df.empty:
        raise SystemExit("No official rear-gap rows available for training.")
    ...
    target_column = "rear_threat_binary_label"
    ...
    model_path = args.output_dir / "rear_threat_model_baseline.txt"
    summary_path = args.output_dir / "rear_threat_baseline_report.json"
...
def scan_thresholds(*, y_true, scores) -> dict:
    """Pick a decision threshold that balances user-visible miss rate and spam rate.

    备注:
    阶段二第一轮更偏“可用性”而不是纯学术最优。
    这里优先选择验证集上 macro F1 更高的阈值，
    同时保留 precision / recall / accuracy 供后续人工校准。
    """
