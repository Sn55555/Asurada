...
DEFAULT_DATASET = PROJECT_ROOT / "training" / "exports" / "phase2_dataset_v1" / "event_features_v1.csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "training" / "reports" / "event_impact_baseline"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the first event-impact baseline model.")
    parser.add_argument("--dataset", type=Path, default=DEFAULT_DATASET, help="Path to event_features_v1.csv.")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> int:
    ...
    df = df[df["event_code"].isin({"COLL", "LGOT", "OVTK", "PENA", "SSTA", "STLG"})].copy()
    if df.empty:
        raise SystemExit("No event rows available for training.")
    ...
    target_column = "event_impact_binary_label"
    ...
    if train_df.empty or test_df.empty:
        raise SystemExit("Insufficient train/test rows after split filtering.")
    ...
    num_boost_round = 150 if generated_holdout_val else 300
    ...
    scores = booster.predict(test_df[feature_columns], num_iteration=best_iteration)
    ...
    model_path = args.output_dir / "event_impact_model_baseline.txt"
    summary_path = args.output_dir / "event_impact_baseline_report.json"
    booster.save_model(str(model_path))

    summary = {
        "dataset": str(args.dataset),
        "filters": {
            "session_type": "RaceLike only",
            "event_codes": ["COLL", "LGOT", "OVTK", "PENA", "SSTA", "STLG"],
        },
        "rows": {
            "all": int(len(df)),
...
def scan_thresholds(*, y_true, scores) -> dict:
    """Pick a threshold that balances recall with acceptable spam."""
