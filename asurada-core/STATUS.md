...
- [ ] live 与 capture replay 的输入源抽象进一步共路径化
...
- [x] 统一下行语音输出主线
- [x] `SpeechJob` 统一语音任务模型
- [x] `SpeechBackend / MacOSSayBackend` 真实播报 backend
- [x] `ConsoleVoiceOutput` 演进为统一输出协调器，并保持 `emit(decision, render=...)` 主入口不变
